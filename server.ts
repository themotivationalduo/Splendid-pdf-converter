    ...
    try {
      ...
      if (!secret) {
        return res.status(400).json({ 
          error: "ConvertAPI secret key is not set in environment. Please configure CONVERT_API_PRODUCTION_SECRET or CONVERT_API_SANDBOX_SECRET in the settings/secrets panel." 
        });
      }
